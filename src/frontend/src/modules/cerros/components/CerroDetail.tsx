      ...
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent flex items-end p-8 md:p-12">
          ...
          >
            <Badge
              variant="primary"
              className="mb-3 backdrop-blur-md bg-primary/90 border-none text-white px-3 py-1 text-sm"
            >
              {cerro.altitude} m
            </Badge>
              ...
              <div>
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {cerro.ecological_value.cultural.map((item, i) => (
                    <li
                      key={i}
                      className="flex items-start gap-2 text-text-muted text-sm p-2 rounded-lg"
                    >
          ...
          <Card className="border-red-200 bg-red-50/50 hover:shadow-md transition-shadow duration-300">
            ...
            <ul className="space-y-3">
              {cerro.threats.map((threat, i) => (
                <li
                  key={i}
                  className="flex items-start gap-2 text-sm text-red-800 bg-white/60 p-2 rounded-md"
                >
